...
def _jump_if_undef_src(ctx: Emit, src: Operand, end: str) -> None:
    idx = src.local_index
    if idx is None or ctx.known_def(idx):
        return
    ctx.jump(f"rt.is_undef({src.lua})", end)
    ctx.mark_def(idx)
...
class ResetLocalStmt:
    ...
    def parse(cls, ctx: Emit, stmt: dict[str, Any]) -> ResetLocalStmt:
        return cls(ctx.local(stmt.get("target"), "ResetLocal target"))

    def emit(self, ctx: Emit, _end: str) -> None:
        ctx.add(f"t{self.target} = rt.UNDEF")
...
class AssignVarStmt:
    ...
    def parse(cls, ctx: Emit, stmt: dict[str, Any]) -> AssignVarStmt:
        return cls(
            ctx.local(stmt.get("target"), "AssignVar target"),
            ctx.operand(stmt.get("source"), "AssignVar source"),
        )

    def emit(self, ctx: Emit, end: str) -> None:
        _jump_if_undef_src(ctx, self.source, end)
        ctx.add(f"t{self.target} = {self.source.lua}")
...
class AssignVarOnceStmt:
    ...
    def parse(cls, ctx: Emit, stmt: dict[str, Any]) -> AssignVarOnceStmt:
        return cls(
            ctx.local(stmt.get("target"), "AssignVarOnce target"),
            ctx.operand(stmt.get("source"), "AssignVarOnce source"),
        )

    def emit(self, ctx: Emit, end: str) -> None:
        _jump_if_undef_src(ctx, self.source, end)
        ctx.add(f'if rt.is_def(t{self.target}) then error("AssignVarOnce conflict") end')
        ctx.add(f"t{self.target} = {self.source.lua}")
        ctx.mark_def(self.target)
